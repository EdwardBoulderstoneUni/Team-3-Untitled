...
void GameTechRenderer::RenderCamera()
{
	...
	for (const auto& i : activeObjects)
	{
		auto shader = static_cast<OGLShader*>((*i).GetShader());
		BindShader(shader);

		BindTextureToShader((*i).GetDefaultTexture(), "mainTex", 0);
		...
		glUniform1i(hasTexLocation, static_cast<OGLTexture*>((*i).GetDefaultTexture()) ? 1 : 0);
