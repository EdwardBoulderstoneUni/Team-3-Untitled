...
bool CollisionDetection::RayCapsuleIntersection(const Ray& r, const Transform& worldTransform,
                                                const CapsuleVolume& volume, RayCollision& collision)
{
	return false;
}
...
Vector3 CollisionDetection::UnprojectScreenPosition(Vector3 position, float aspect, float fov, const Camera& c)
{
	...
	//and then subtracting 1 (-1.0 to 1.0).
	auto clipSpace = Vector4(
		(position.x / screenSize.x) * 2.0f - 1.0f,
		(position.y / screenSize.y) * 2.0f - 1.0f,
		(position.z) - 1.0f,
		1.0f
	);
...
bool CollisionDetection::ObjectIntersection(GameObject* a, GameObject* b, CollisionInfo& collisionInfo)
{
	...
	auto pairType = static_cast<VolumeType>(static_cast<int>(volA->type) | static_cast<int>(volB->type));
