...
void Debug::DrawAxisLines(const Matrix4& modelMatrix, float scaleBoost, float time)
{
	...
	DrawLine(worldPos, worldPos + (right * scaleBoost), RED, time);
	DrawLine(worldPos, worldPos + (up * scaleBoost), GREEN, time);
	DrawLine(worldPos, worldPos + (fwd * scaleBoost), BLUE, time);
}
