...
NavigationGrid::NavigationGrid(const std::string& filename) : NavigationGrid()
{
	...
	for (int y = 0; y < gridHeight; ++y)
	{
		for (int x = 0; x < gridWidth; ++x)
		{
			...
			infile >> type;
			n.type = type;
			n.position = Vector3(static_cast<float>(x * nodeSize), 0, static_cast<float>(y * nodeSize));
		}
	}
...
bool NavigationGrid::FindPath(const Vector3& from, const Vector3& to, NavigationPath& outPath)
{
	...
	int fromX = (static_cast<int>(from.x) / nodeSize);
	int fromZ = (static_cast<int>(from.z) / nodeSize);

	int toX = (static_cast<int>(to.x) / nodeSize);
	int toZ = (static_cast<int>(to.z) / nodeSize);
