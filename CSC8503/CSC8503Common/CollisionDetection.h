...
namespace NCL
{
	...
	public:
		...
		struct CollisionInfo
		{
			...
			{
				size_t otherHash = static_cast<size_t>(other.a->GetWorldID()) + (static_cast<size_t>(other.b->
					GetWorldID()) << 32);
				size_t thisHash = static_cast<size_t>(a->GetWorldID()) + (static_cast<size_t>(b->GetWorldID()) << 32);
		...
		static bool RayBoxIntersection(const Ray& r, const Vector3& boxPos, const Vector3& boxSize,
		...
		static bool RayAABBIntersection(const Ray& r, const Transform& worldTransform, const AABBVolume& volume,
		                                RayCollision& collision);
		static bool RayOBBIntersection(const Ray& r, const Transform& worldTransform, const OBBVolume& volume,
		                               RayCollision& collision);
		static bool RaySphereIntersection(const Ray& r, const Transform& worldTransform, const SphereVolume& volume,
		                                  RayCollision& collision);
		static bool RayCapsuleIntersection(const Ray& r, const Transform& worldTransform, const CapsuleVolume& volume,
		                                   RayCollision& collision);


		static bool RayPlaneIntersection(const Ray& r, const Plane& p, RayCollision& collisions);

		static bool AABBTest(const Vector3& posA, const Vector3& posB, const Vector3& halfSizeA,
