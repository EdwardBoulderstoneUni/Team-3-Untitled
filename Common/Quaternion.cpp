...
Quaternion::Quaternion(const Matrix4& m)
{
	...
	if (abs(w) < 0.0001f)
	{
		...
		x = copysign(x, m.array[9] - m.array[6]);
		y = copysign(y, m.array[2] - m.array[8]);
		z = copysign(z, m.array[4] - m.array[1]);
	}
	else
	{
		float qrFour = 4.0f * w;
...
{
	float cos1 = cos(DegreesToRadians(yaw * 0.5f));
	float cos2 = cos(DegreesToRadians(pitch * 0.5f));
	float cos3 = cos(DegreesToRadians(roll * 0.5f));

	float sin1 = sin(DegreesToRadians(yaw * 0.5f));
	float sin2 = sin(DegreesToRadians(pitch * 0.5f));
	float sin3 = sin(DegreesToRadians(roll * 0.5f));
...
Quaternion Quaternion::AxisAngleToQuaterion(const Vector3& vector, float degrees)
{
	...
	float result = sin(theta / 2.0f);

	return Quaternion(vector.x * result, vector.y * result, vector.z * result, cos(theta / 2.0f));
}
