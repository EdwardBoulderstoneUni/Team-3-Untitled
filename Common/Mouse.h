...
namespace NCL
{
	...
	public:
		...
		{
			return buttons[static_cast<int>(button)] && !holdButtons[static_cast<int>(button)];
		}

		//Is this mouse button currently pressed down?
		bool ButtonDown(MouseButtons button) const { return buttons[static_cast<int>(button)]; }
		//Has this mouse button been held down for multiple frames?
		bool ButtonHeld(MouseButtons button) const
		{
			return buttons[static_cast<int>(button)] && holdButtons[static_cast<int>(button)];
		}

		//Has this mouse button been double clicked?
		bool DoubleClicked(MouseButtons button) const
		{
			return (buttons[static_cast<int>(button)] && doubleClicks[static_cast<int>(button)]);
		}
	...
	protected:
		...
		bool buttons[static_cast<int>(MouseButtons::MAXVAL)];
