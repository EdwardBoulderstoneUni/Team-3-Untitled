...
enum class GeometryChunkData
{
	...
	//Just float data
	dShort,
	//Translate from -32k to 32k to a float
	dByte,
	//Translate from -128 to 127 to a float
};
...
void* ReadVertexData(GeometryChunkData dataType, GeometryChunkTypes chunkType, int numVertices)
{
	...
	auto data = new char[numElements * bytesPerElement];
...
bool MeshGeometry::HasTriangle(unsigned int i) const
{
	...
	return i < static_cast<unsigned>(triCount);
}
