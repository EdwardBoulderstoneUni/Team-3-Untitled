...
{
	ENetRangeCoder* rangeCoder = enet_malloc(sizeof(ENetRangeCoder));
	if (rangeCoder == NULL)
		return NULL;
...
{
	ENetRangeCoder* rangeCoder = context;
	if (rangeCoder == NULL)
		return;
...
size_t
enet_range_coder_compress(void* context, const ENetBuffer* inBuffers, size_t inBufferCount, size_t inLimit,
                          enet_uint8* outData, size_t outLimit)
{
	...
	for (;;)
	{
		...
		*parent = symbol - rangeCoder->symbols;
		...
		if (count > 0xFF - 2 * ENET_CONTEXT_SYMBOL_DELTA + ENET_CONTEXT_SYMBOL_MINIMUM || root->total >
			ENET_RANGE_CODER_BOTTOM - 0x100)
...
size_t
enet_range_coder_decompress(void* context, const enet_uint8* inData, size_t inLimit, enet_uint8* outData,
                            size_t outLimit)
{
	ENetRangeCoder* rangeCoder = context;
	enet_uint8 *outStart = outData, *outEnd = &outData[outLimit];
