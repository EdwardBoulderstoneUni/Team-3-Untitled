...
enet_protocol_notify_connect(ENetHost* host, ENetPeer* peer, ENetEvent* event)
{
	...
	if (event != NULL)
	{
		...
		enet_protocol_dispatch_state(host, peer, peer->state == ENET_PEER_STATE_CONNECTING
                                      ...
                                      enet_uint8** currentData)
{
	...
	*currentData += dataLength;
	...
	if (unsequencedGroup >= (enet_uint32)peer->incomingUnsequencedGroup + ENET_PEER_FREE_UNSEQUENCED_WINDOWS *
                                     ...
                                     enet_uint8** currentData)
{
	...
	*currentData += dataLength;
	...
	if (enet_peer_queue_incoming_command(peer, command, (const enet_uint8*)command + sizeof(ENetProtocolSendUnreliable),
	                                     dataLength, 0, 0) == NULL)
		return -1;
...
static int
enet_protocol_handle_send_unreliable_fragment(ENetHost* host, ENetPeer* peer, const ENetProtocol* command,
                                              enet_uint8** currentData)
{
	enet_uint32 fragmentNumber,
	            fragmentCount,
...
{
	if (peer->state == ENET_PEER_STATE_DISCONNECTED || peer->state == ENET_PEER_STATE_ZOMBIE || peer->state ==
		ENET_PEER_STATE_ACKNOWLEDGING_DISCONNECT)
		return 0;

	enet_peer_reset_queues(peer);

	if (peer->state == ENET_PEER_STATE_CONNECTION_SUCCEEDED || peer->state == ENET_PEER_STATE_DISCONNECTING || peer->
		state == ENET_PEER_STATE_CONNECTING)
		enet_protocol_dispatch_state(host, peer, ENET_PEER_STATE_ZOMBIE);
...
enet_protocol_send_reliable_outgoing_commands(ENetHost* host, ENetPeer* peer)
{
	...
	while (currentCommand != enet_list_end(& peer -> outgoingReliableCommands))
	{
			...
			if (! windowWrap &&
				...
				(channel->reliableWindows[(reliableWindow + ENET_PEER_RELIABLE_WINDOWS - 1) %
					...
					channel->usedReliableWindows & ((((1 << ENET_PEER_FREE_RELIABLE_WINDOWS) - 1) << reliableWindow) |
						(((1 << ENET_PEER_FREE_RELIABLE_WINDOWS) - 1) >> (ENET_PEER_RELIABLE_WINDOWS -
							reliableWindow)))))
				windowWrap = 1;
			if (windowWrap)
			{
		...
		if (command >= &host->commands[sizeof (host->commands) / sizeof(ENetProtocol)] ||
			...
			peer->mtu - host->packetSize < commandSize ||
			(outgoingCommand->packet != NULL &&
				(enet_uint16)(peer->mtu - host->packetSize) < (enet_uint16)(commandSize + outgoingCommand->
					fragmentLength)))
		{
			host->continueSending = 1;
...
enet_protocol_send_outgoing_commands(ENetHost* host, ENetEvent* event, int checkForTimeouts)
{
	...
	ENetProtocolHeader* header = headerData;
	...
	while (host->continueSending)
		     ...
		     ++ currentPeer)
		{
			...
			{
				enet_uint32 packetLoss = currentPeer->packetsLost * ENET_PEER_PACKET_LOSS_SCALE / currentPeer->
					packetsSent;
