...
GLuint shaderTypes[static_cast<int>(ShaderStages::SHADER_MAX)] = {
	GL_VERTEX_SHADER,
	GL_FRAGMENT_SHADER,
	GL_GEOMETRY_SHADER,
...
string ShaderNames[static_cast<int>(ShaderStages::SHADER_MAX)] = {
	"Vertex",
	"Fragment",
	"Geometry",
	"Tess. Control",
...
OGLShader::OGLShader(const string& vertex, const string& fragment, const string& geometry, const string& domain,
                     const string& hull) :
	ShaderBase(vertex, fragment, geometry, domain, hull)
{
	for (int i = 0; i < static_cast<int>(ShaderStages::SHADER_MAX); ++i)
	{
		shaderIDs[i] = 0;
		shaderValid[i] = 0;
	}
	...
	for (int i = 0; i < static_cast<int>(ShaderStages::SHADER_MAX); ++i)
	{
		if (!shaderFiles[i].empty())
		{
			if (Assets::ReadTextFile(Assets::SHADERDIR + shaderFiles[i], fileContents))
			{
				...
				int stringLength = static_cast<int>(fileContents.length());
...
void OGLShader::PrintCompileLog(GLuint object)
{
	...
	if (logLength)
	{
		auto tempData = new char[logLength];
		glGetShaderInfoLog(object, logLength, nullptr, tempData);
...
void OGLShader::PrintLinkLog(GLuint program)
{
	...
	{
		auto tempData = new char[logLength];
		glGetProgramInfoLog(program, logLength, nullptr, tempData);
		std::cout << "Link Log:\n" << tempData << std::endl;
		delete tempData;
	}
}
